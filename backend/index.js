"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var Express = require("express");
var Cors = require("cors");
var Dotenv = require("dotenv");
var Fs = require("fs");
var Qr = require('qrcode');
var _a = require('whatsapp-web.js'), Client = _a.Client, NoAuth = _a.NoAuth;
Dotenv.config();
var connected = 0;
var whaqr;
var message;
var messages = [];
var client = new Client({
    authStrategy: new NoAuth()
});
// Variaveis ETC (Edite Para Configurar)
console.log("Servidor VickWEB (API/BACKEND)");
console.log("");
client.on("qr", function (qr) {
    whaqr = qr;
    console.log("Waiting Connection...");
    messages.push({ form: "INFO", type: "text", body: "Espere/Escaneie O QR Code" });
});
var url = process.env.URL || "localhost";
var porta = process.env.PORTA || 3001;
console.log("Set Vars!");
// Vars FIM
client.on("ready", function () {
    message = "Whatsapp Loading";
    console.log("LOADING...");
    messages.push({ form: "INFO", type: "text", body: "Conected!" });
    whaqr = "";
    connected = 1;
});
client.on("message", function (msg) { return __awaiter(void 0, void 0, void 0, function () {
    var contato, media, err_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, msg.getContact()];
            case 1:
                contato = _a.sent();
                _a.label = 2;
            case 2:
                _a.trys.push([2, 6, , 7]);
                if (!msg.hasMedia) return [3 /*break*/, 4];
                return [4 /*yield*/, msg.downloadMedia()];
            case 3:
                media = _a.sent();
                messages.push({ from: contato.pushname || contato.number, type: "media", mimetype: media.mimetype, filename: media.filename, data: media.data });
                return [3 /*break*/, 5];
            case 4:
                messages.push({ from: contato.pushname + " | " + contato.number, type: "text", body: msg.body });
                _a.label = 5;
            case 5: return [3 /*break*/, 7];
            case 6:
                err_1 = _a.sent();
                messages.push({ from: "ERROR", type: "text", body: "the message not send due to an error" });
                return [3 /*break*/, 7];
            case 7: return [2 /*return*/];
        }
    });
}); });
var app = Express();
app.use(Cors());
app.use(Express.json());
console.log("WEB LOADED!");
// Connection Server
app.get("/ping", function (req, res) {
    res.json({ msg: "Conected!" });
});
app.get("/qr", function (req, res) {
    if (!whaqr) {
        return res.send("403");
    }
    res.setHeader("Content-Type", "image/png");
    Qr.toFileStream(res, whaqr);
});
app.get("/logout", function (req, res) {
    if (connected == 1) {
        client.destroy().then(function () {
            Fs.rmSync('./../.wwebjs_auth', { recursive: true, force: true });
            connected = 0;
            client.initialize();
            messages.push({ form: "INFO", type: "text", body: "Logout!" });
        });
    }
    else {
        messages.push({ form: "INFO", type: "text", body: "Already Logouted!" });
    }
});
app.post("/env", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, numero, mensagem, err_2;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                if (!req.body) return [3 /*break*/, 7];
                _a = req.body, numero = _a.numero, mensagem = _a.mensagem;
                if (!(!numero || !mensagem)) return [3 /*break*/, 1];
                messages.push({ form: "ERROR", type: "text", body: "Number or Message Empty!" });
                return [3 /*break*/, 7];
            case 1:
                if (!(connected == 1)) return [3 /*break*/, 6];
                _b.label = 2;
            case 2:
                _b.trys.push([2, 4, , 5]);
                return [4 /*yield*/, client.sendMessage(numero + "@c.us", mensagem)];
            case 3:
                _b.sent();
                messages.push({ form: "INFO", type: "text", body: "Message Sended! Number:" + numero + "." });
                return [3 /*break*/, 5];
            case 4:
                err_2 = _b.sent();
                messages.push({ form: "ERROR", type: "text", body: "ERROR WHILE TRYING TO SEND MESSAGE" });
                return [3 /*break*/, 5];
            case 5: return [3 /*break*/, 7];
            case 6:
                messages.push({ form: "ERROR", type: "text", body: "NOT CONNECTED" });
                _b.label = 7;
            case 7: return [2 /*return*/];
        }
    });
}); });
app.get("/msg", function (req, res) {
    res.json(messages);
});
app.post("/clear", function (req, res) {
    messages = [];
});
console.log("URL Loaded!");
console.log("");
console.log("");
console.log("");
app.listen(porta, function () {
    console.log("Started With Success! Port:".concat(porta, ", URL localhost"));
    console.log("MENSAGEM DO SERVIDOR:");
    console.log("Algumas Vezes, o Codigo Falha com Sucesso");
});
client.initialize();
